...
import tp.pr4.utils.StringUtils;
...
/**
 * 
 * Clase encargada de cargar la información de la ciudad de un archivo
 */
public class CityLoaderFromTxtFile {
	...
	private Scanner readPlaces(Scanner sc) throws WrongCityFormatException{
		...
		if(!ok) throw new WrongCityFormatException(); //lanza la excepcion si el formato es incorrecto
		return sc;
	}
	...
	private boolean correctStreets(String[] WORD, int i){
		boolean ok = true;
		if((WORD.length >= 8) && WORD[0].equals("street") && StringUtils.isInteger(WORD[1]) && (Integer.parseInt(WORD[1]) == i) && (WORD[2].equals("place")) &&
	...
	private boolean correctStreectSecondPart(String[] WORD){
		boolean ok = true;
		if(WORD[5].equals("place") && StringUtils.isInteger(WORD[3]) && StringUtils.isInteger(WORD[6]) &&
	...
	private boolean correctPlaces(String[] WORD, int i){
		boolean ok = true;
		if((WORD.length == 5) && WORD[0].equals("place") && StringUtils.isInteger(WORD[1]) && (Integer.parseInt(WORD[1]) == i) && //si las siguientes palabras son correcctas y si la longitud del array es igual a cinco
			((WORD[4].equals("noSpaceShip")) || WORD[4].equals("spaceShip"))){
				this.places.add(new Place(WORD[2], (WORD[4].equals("spaceShip")? true:false), StringUtils.ingoreUnderLines(WORD[3])));
		}
		else ok = false;
		return ok;
	}
	...
	private boolean correctItems(String[] WORD, int i){
		return ((WORD.length >= 7) &&  (WORD[0].equals("fuel") || WORD[0].equals("codecard") || WORD[0].equals("garbage")) &&
				StringUtils.isInteger(WORD[1]) && (Integer.parseInt(WORD[1]) == i)) && //si la longitud del array es mayor o igual a siete y si las palabras son correctas
	...
	private boolean correctFuel(String[] WORD){
		boolean ok = true;
		if(WORD[0].equals("fuel") && WORD[6].equals("place") && (WORD.length == 8) && StringUtils.isInteger(WORD[7]) && (Integer.parseInt(WORD[7]) < this.places.size())
				&& StringUtils.isInteger(WORD[4]) && StringUtils.isInteger(WORD[5])){ //si es fuel y todo es correcto
			this.places.get(Integer.parseInt(WORD[7])).addItem(new Fuel(WORD[2], StringUtils.ingoreUnderLines(WORD[3]), Integer.parseInt(WORD[4]), Integer.parseInt(WORD[5])));
		}
		else ok = false;
		return ok;
	}
	...
	private boolean correctGarbage(String[] WORD){
		boolean ok = true;
		if(WORD[0].equals("garbage") && WORD[5].equals("place") && (WORD.length == 7) && StringUtils.isInteger(WORD[6]) && (Integer.parseInt(WORD[6]) < this.places.size())
				&& StringUtils.isInteger(WORD[4])){ //si es garbage y todo es correcto
			this.places.get(Integer.parseInt(WORD[6])).addItem(new Garbage(WORD[2], StringUtils.ingoreUnderLines(WORD[3]), Integer.parseInt(WORD[4])));
		}
		else ok = false;
		return ok;
	}
	...
	private boolean correctCodeCard(String[] WORD){
		boolean ok = true;
		if(WORD[0].equals("codecard") && WORD[5].equals("place") && StringUtils.isInteger(WORD[6])){ //si es CodeCard y todo es correcto
			this.places.get(Integer.parseInt(WORD[6])).addItem(new CodeCard(WORD[2], StringUtils.ingoreUnderLines(WORD[3]), WORD[4]));
		}
		else ok = false;
		return ok;
	}
